...
def drawGrid(x_0, y_0, x, y, color = white, width=5, screen = screen):
...
def drawX(x_0, y_0, x, y, color = white, width = 5, screen = screen):
...
def drawO(x_0, y_0, x, y, color = white, width = 5, screen = screen):
    rect = (x_0, y_0, x, y) #retangulo em que a bola é centrada
    pygame.draw.arc(screen, color, rect, 0, 2*pi, width)


def drawGame(x_0, y_0, x, y, game, fill = .8, OColor = blue, XColor = red, gridColor = white,
             screen = screen, XWidth = 5, OWidth = 5, gridWidth = 5):
    ...
    #desenha o tabuleiro
    drawGrid(x_0, y_0, x, y, gridColor, gridWidth, screen)
    ...
    for line in range(3):
        for column in range(3):
            if game[line][column] == 'E':
                ...
                drawX(X_0[column], Y_0[line], rect_x, rect_y, XColor, XWidth, screen)
            elif game[line][column] == 'O':
                drawO(X_0[column], Y_0[line], rect_x, rect_y, OColor, OWidth, screen)
            else:
...
def drawWinLine(win, x_0, y_0, x, y, color = white, width = 5, screen = screen):
    if win[0] == 'E':
        return True
    winPos = win[1]
...
while test:
    ...
    screen.fill(black)
    drawGame(x_0, y_0, x, y, game, OWidth = 3)
    win = checkWinner(game)
    drawWinLine(win, x_0, y_0, x, y)
    pygame.display.flip()
    clock.tick(60)
